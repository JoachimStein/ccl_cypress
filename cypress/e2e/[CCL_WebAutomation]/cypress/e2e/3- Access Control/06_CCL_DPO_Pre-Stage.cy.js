    ...
    user_ids.forEach((user, index) => {
        it(`User for Direct Purchase Order : ${user.role}`, () => {
            ...
            if (userIndex === 0) {
                ...
                cy.wait(1000)
                cy.get("#direct_po_base_0").select('Note Sheet')    //select purchase base
                cy.wait(1000)
                ...
                cy.wait(1000) 
                // cy.get("#rfq_id_0").click() // RFQ
                // cy.get("#rfq_id_0").type('RFQ/00').type('{Enter}')
                // cy.wait(1000)               
                cy.get("table[class='o_section_and_note_list_view o_list_table table table-sm table-hover position-relative mb-0 o_list_table_ungrouped table-striped']>tbody>tr:first-child>td:nth-child(11)").type("20")
                cy.wait(1000)
