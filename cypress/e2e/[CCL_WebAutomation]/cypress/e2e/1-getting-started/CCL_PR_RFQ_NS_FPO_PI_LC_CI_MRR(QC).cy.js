describe("mrr(qc)_testsuite", function(){
    it("process_flow_FPO", function(){
        ...
        cy.wait(1000)
        cy.get("#subject_0").type("Automated Testing: PR>RFQ>Q>NS(RFQ)>FPO>PI>LC>CI>MRR(QC)")
        cy.wait(1000)
        cy.get("#body_0").type("Automated Testing: PR>RFQ>Q>NS(RFQ)>FPO>PI>LC>CI>MRR(QC)")
        cy.wait(1000)
        ...
        cy.wait(500)
        cy.get("#proforma_invoice_id_0").type('{Enter}')   //PI select
        cy.wait(1000)
        cy.xpath("//span[contains(.,'Cancel')]").should('have.text', 'Cancel')
        cy.xpath("//span[contains(.,'Confirm')]").click()        
        cy.wait(2000)
